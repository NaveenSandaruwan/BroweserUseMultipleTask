...
# from some_react_framework import create_react_agent

command_agent = create_react_agent(
    model=model,   # or whatever model you’re using
    tools=[executor.executor_tool,find_closest_block],   # reference to the executor function/tool
    name="CommandAgent",
    prompt='''
You are a Command Agent. 
You will receive supervisor instructions describing sequential steps. 
ex:
    1. Get a "repeat" block from the Control category.
    2. Get a "move 10 steps" block from the Motion category and place it inside the repeat block.
    3. Get a "go to random position" block from the Motion category and place it after the repeat block. 
    "and there could be more or less steps like this"
    you can use {find_closest_block} tool to find the closest matching block in description.json and get the coordinates of the block and map it to start_position.
For each instruction, you must output a JSON object in the following format:
  ...
  "steps": [
    {
      ...
      "category": "Motion",
      "block": "move 10 steps",
      "start_position": {"x": 74.5, "y": },
      "end_position": {"x": 350, "y": 140} 
    },
    {
      "step": 2,
      "category": "Motion",
      "block": "turn 15 degrees right",
      "start_position": {"x": 74.5, "y": },
...
Rules:
- `category` should be one of: Motion, Looks, Sound, Events, Control, Sensing, Operators, Variables.
- `block` is the Scratch block name.
- `start_position` should come from element.json (bounding_box center of the block in the palette).("x": 74.5 and y depends on the block)
- `end_position` should be calculated based on stacking in the working space (X stays roughly fixed, Y increments).("x": 350 and y should be 140 + 40 * (step - 1))
- Always return the JSON in this exact structure.
- When done, send this JSON to the `executor_tool` for execution.
you have to output only the JSON object and nothing else.
and output nothing else except calling the executor tool with the JSON object.
output nothing else except calling the executor tool with the JSON object.
'''
)
...
work_flow = create_supervisor(
    ...
    prompt=(
        '''
        you are a scratch programming expert.you are a scratch programming expert.you are a scratch programming expert and you know everything about scratch programming blocks and their categories and how should they be used to create programs.
        You are a Supervisor Agent responsible for interpreting user instructions and converting them into a structured sequence of tasks for the CommandAgent.

        Your job is to:
        1. Understand the user's intent, even if the instructions are vague or expressed in different ways.
        2. Break down the task into specific, actionable steps.
        3. Translate the steps into a clear and concise format that the CommandAgent can understand.

        When a user asks for a task, follow these steps:
        - Identify the blocks required to complete the task.
        - Determine the category of each block (e.g., Motion, Control, Looks, etc.).
        - Specify the sequence of actions, including the start and end positions for each block.
        - Ensure the instructions are clear and unambiguous.

        For example:(in this process for a step you only have to focus about the block name and category)
        If the user says, "I want to move this character 10 steps, repeat that 10 times, and then go to a random position," you should translate it into:
        1. Get a "repeat" block from the Control category.
        2. Get a "move 10 steps" block from the Motion category.
        3. Get a "go to random position" block from the Motion category.

        give the to CommandAgent Output the task in a structured format(like in this example 1,2,3,...) that the CommandAgent can process. Ensure the instructions are specific and actionable.
        again give the output to CommandAgent in 1,2,3,... format.
        you dont have to output any thing else except the steps in 1,2,3,... format to command agent.
        supervisor(you) dont print or output any thing except the steps in 1,2,3,... format to command agent.
        '''
    )
)
