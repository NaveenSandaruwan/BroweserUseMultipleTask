...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	async def continuous_browser_control(self, initial_task: str = None, max_steps_per_task: int = 20) -> None:
		...
		"""
		if initial_task:
			# If initial task provided, override the current task
			self.task = initial_task
			self._message_manager.add_new_task(initial_task)
		...
		while True:
			try:
				...
				# Reinitialize eventbus for each new task to prevent QueueShutDown errors
				try:
					from bubus import EventBus
					self.eventbus = EventBus(name=f'Agent_{str(self.id)[-4:]}')
				except Exception as e:
				...
				try:
					result = await self.run(max_steps=max_steps_per_task)
					print("\n✅ Task completed. Browser session remains active.")
				except bubus.service.QueueShutDown:
					print("\n⚠️ Event queue was shut down. Reinitializing for next task.")
					continue
				except Exception as e:
					print(f"\n❌ Error: {e}")
					if "QueueShutDown" in str(e):
						print("Event queue was shut down. Reinitializing for next task.")
						continue
