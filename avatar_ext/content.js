...
function simpleMarkdownToHtml(markdownText) {
  ...
  // 1. Initialize UI elements at the top level
  const { micBtn, statusEl, emotionBtn } = createAvatarUI();
  
  if (!SpeechRecognitionAPI) {
    alert("Speech recognition not supported in this browser");
  } else if (micBtn) { // 2. Add crucial check for micBtn existence
  ...
    // Error fix: micBtn is now guaranteed to be non-null here
  ...
    };
      ...
      // This handles the case where SpeechRecognition is supported but UI creation failed
      statusEl.textContent = "Fatal Error: UI not fully loaded.";
  }
  ...
  // This redundant call is removed as it's now initialized above the conditional block.
  // createAvatarUI();