...
function createAvatar() {
  ...
  `;
  // Add hover effect
  micButton.onmouseover = () => { micButton.style.transform = 'scale(1.1)'; };
  micButton.onmouseout = () => { micButton.style.transform = 'scale(1)'; };
...
function setupSpeechRecognition(micButton, micStatus) {
  if (!('webkitSpeechRecognition' in window)) {
    console.error("Python Avatar: Speech recognition not supported in this browser");
    micStatus.textContent = "Speech recognition not supported";
    micStatus.style.opacity = 1;
    setTimeout(() => { micStatus.style.opacity = 0; }, 3000);
    return;
  }
  ...
  recognition.onend = () => {
    ...
    setTimeout(() => { micStatus.style.opacity = 0; }, 1000);
  };
